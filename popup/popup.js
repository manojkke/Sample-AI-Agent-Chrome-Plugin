document.addEventListener("DOMContentLoaded", function () {
  ...
  sendBtn.addEventListener("click", function () {
    debugger;
    chrome.runtime.sendMessage({ action: "accessDOM" });
  ...
  // Listen for the title sent back from the content script
  chrome.runtime.onMessage.addListener((message) => {
    debugger;

    if (message.action === "updatePopup") {
      displayMessage("user", message.title);
    }
  });
  ...
  function displayAssistanInfo() {
    ...
    text.innerText = "Hi there, this is Manoj...";
    text.className = "assistant-info-text";
    messageElement.appendChild(text);

    // Create a p element for the text
    const text2 = document.createElement("p");
    text2.innerText = "Coming Soon with awesome AI tool ...";
    text2.className = "assistant-info-text";
    messageElement.appendChild(text2);
